    ...
    private void addEntriesRelatedToInvoice(long invoiceId, Invoice invoice) {
        ...
        invoice.getEntries().forEach(entry -> {
            jdbcTemplate.update(connection -> {
                PreparedStatement ps = connection
                    .prepareStatement(
                        "insert into invoice_entry (description, quantity, price, vat_value, vat_rate, expense_related_to_car) "
                            + "values (?, ?, ?, ?, ?, ?);",
                        new String[] {"id"});
                ...
                ps.setObject(6, insertCarAndGetItId(entry.getCarRelatedExpense()));
                return ps;
            }, keyHolder);

            long invoiceEntryId = keyHolder.getKey().intValue();
